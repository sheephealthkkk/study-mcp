...
2. **客户端发现与决策**：客户端向服务器请求所有可用的Resources列表。然后，客户端根据当前的上下文或用户的指令，**自主决定**需要加载哪些Resources来帮助模型更好地回答问题。
...
3. **用户选择与调用**：用户通过点击或输入命令的方式主动选择一个 Prompt。客户端会向服务器请求该 Prompt 的具体内容。如果需要，用户还可以填充一些参数（如指定要分析的代码）。服务器收到请求后，会根据模板和用户提供的参数，动态生成最终的消息内容并返回给客户端。最后，客户端将这个准备好的消息发送给大语言模型（LLM）。
   ...
   **用户主导，而非模型或程序**：与 MCP 中的“Tools”不同，“Tools”是由 LLM 自己决定何时调用的。而 **Prompts 的调用完全由用户主动触发**，用户拥有最终决定权
...
- **通知（Notification）**：**不包含 `id` 字段**。接收方收到后不会回复任何消息。
